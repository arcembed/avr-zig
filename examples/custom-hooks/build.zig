...
const avr_board = struct {
    ...
    pub fn spec(board: Board) Spec {
        return switch (board) {
            .uno => .{
                .target_query = .{ .cpu_arch = .avr, .cpu_model = .{ .explicit = &std.Target.avr.cpu.atmega328p }, .os_tag = .freestanding, .abi = .none },
                .linker_script = "src/runtime/atmega328p.ld",
            },
            .mega2560 => .{
                .target_query = .{ .cpu_arch = .avr, .cpu_model = .{ .explicit = &std.Target.avr.cpu.atmega2560 }, .os_tag = .freestanding, .abi = .none },
    ...
    pub fn addMonitorStep(b: *std.Build, tty: []const u8) void {
        ...
        const screen = b.addSystemCommand(&.{ "screen", tty, "115200" });
...
pub fn build(b: *std.Build) void {
    ...
    avr_board.addUploadStep(b, board, tty, "Flash the custom hooks example with avrdude", bin_path);
    avr_board.addObjdumpStep(b, "Disassemble the custom hooks firmware", bin_path);
